...
def project(request):
    ...
    if request.method== "POST":
        ...
        if proj_name== "":
            # 未输入直接点击查询，返回所有模块
            proj_name= ""
            projects= Project.objects.order_by("-create_time")
        else:
            projects= Project.objects.filter(name__contains= proj_name).order_by("-create_time") # 模糊查询所有项目名
    else:
        proj_name= ""
        projects= Project.objects.order_by("-create_time") # 根据创建时间倒序
...
def module(request):
    ...
    if request.method== "POST":
        ...
        if proj_name== "":
            # 未输入直接点击查询，返回所有模块
            proj_name= ""
            modules= Module.objects.order_by("-create_time")
        else:
            projects= Project.objects.filter(name__contains= proj_name) # 模糊查询所有项目名
            projects_id= [i.id for i in projects] # 找到项目对应id
            modules= Module.objects.filter(belong_project__in= projects_id).order_by("-create_time")
            # modules= Module.objects.filter(belong_project__module__in= projects_id).order_by("-create_time")
            # # belong_project__module__in 没有将模块去重
    else:
        proj_name= ""
        modules= Module.objects.order_by("-create_time") # 根据创建时间倒序
    ...
    if request.method== "GET":
        ...
        if not ex_case:
            ...
            if case_name in [None, ""]:
                case_name= ""
                cases= TestCase.objects.filter(status= 0).order_by("-create_time")
            else:
...
def up_test_template(request):
    ...
    cases= TestCase.objects.filter(status= 0).order_by("-create_time")  # 上传完成后，自动跳回用例页面
    data= {}
    data["pages"]= get_paginator(request, cases)
    data["case_name"]= ""
    return render(request,"test_case.html",data)
...
def module_test_case(request,moduleid):
    '''主页-模块，关联对应测试用例'''
    if moduleid:
        cases= TestCase.objects.filter(belong_module_id= moduleid,status= 0).order_by("-create_time")
    else:
        cases= TestCase.objects.filter(status= 0).order_by("-create_time")
    ...
    # 点击执行后，生成集合执行记录，集合执行记录包含用例执行记录
        ...
        if not ex_suite:
            ...
            if suite_name in [None, ""]:
                suite_name= ""
                suites= TestSuite.objects.filter(status= 0).order_by("-create_time")
            else:
...
def module_test_case_statistics(request,module_id):
    ...
    :return:
    '''
    cases= TestCase.objects.filter(status= 0,belong_module= module_id).order_by("-create_time")
...
def project_test_case_statistics(request,project_id):
    ...
    :return:
    '''
    cases= TestCase.objects.filter(status= 0,belong_project= project_id).order_by("-create_time")
