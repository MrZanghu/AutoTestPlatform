    ...
    def setUp(self):
        ...
        if self.code== 0:
            ...
            try:
                ...
                self.execute_record.execute_result= "失败"
                logger.info("接口请求异常，error:{}".format(traceback.format_exc(limit=3)))  # 只向上找3层级
            ...
            if self.assert_key!= None:
                ...
                for key_word in self.assert_key_list:
                    if not (key_word in json.dumps(self.res_data.json(), ensure_ascii= False)):
                        logger.info("断言关键字【{}】匹配失败".format(key_word))
    ...
    def tearDown(self):
        ...
        try:
            if self.result_flag:
                ...
                logger.info("用例【%s】执行失败,请检查错误！" % self.test_case)
        except Exception as e:
            # 提取关键字错误时执行
            self.execute_record.extract_var= None
            logger.info("用例【%s】执行失败,请检查错误！" % self.test_case)
...
def process_xls(up_times,owner,file_names):
    ...
    try:
        ...
        if not p:
            ...
            logger.info("已存在相同项目,名称为【{}】".format(project_name))
        ...
        if not m:
            ...
            logger.info("已存在相同模块,名称为【{}】".format(module_name.name))
